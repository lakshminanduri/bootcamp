...
import com.backbase.android.retail.journey.app.universal.UniversalUseCaseDefinitions
...
fun ModelBankUseCaseDefinitions(overrides: UniversalUseCaseDefinitions.Builder.() -> Unit = {}): UniversalUseCaseDefinitions {
    return DefaultUseCaseDefinitions().apply(overrides).build()
}

@Suppress("FunctionName", "RemoveExplicitTypeArguments")
internal fun DefaultUseCaseDefinitions() = UniversalUseCaseDefinitions.Builder().apply {

    accountsBannerUseCaseDefinition = {
        if (getOrNull<AccountsAndTransactionsConfiguration>()?.accountsScreen?.isBannersEnabled == true) {
            UniversalEngagementAccountsBannerUseCase(
...
private class UniversalEngagementAccountsBannerUseCase(
